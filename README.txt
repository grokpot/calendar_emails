...
3. calendars_to_process.csv: contains specific calendar IDs to process. Useful if you have an organization that conatins lots of calendars and you only want to process a few. Place the emails in new rows in col1.

4. domains_to_exclude.csv: contains domains to exclude from list of new contacts. Useful if you want to exclude things like your organization's emails, specific clients, dev shops, etc.

5. Run the program with '$ python calendar_emails.py'. Select the Google account you want to process calendars for.

6. Emails and names that do not exist in master_list.csv are written to a csv file called new_contacts.csv. This file is overwritten every time the program runs.
